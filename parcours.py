    ...
    for a in vars(args):
        ...
        if a in conf:
            if attr:  # append the two in the case of multivalueds
                ...
                if a == 'extant':
                    # Remove duplicates
                    setattr(args, a, list(dict.fromkeys(attr + conf[a])))
                if a == 'cost':
...
def file_selection_gui():
    ...
    my_w.title('Ancestral Information')
    my_font1 = ('times', 18, 'bold')

    l1 = tk.Label(my_w, text='Upload File', width=30, font=my_font1)
    ...
    l3 = tk.Label(my_w, textvariable=my_str2, fg='blue')
    l3.grid(row=5, column=1)
    # my_str2.set("")

    b2 = tk.Button(my_w, text='Run',
                   width=20, command=lambda: close_window())
...
def process_args(args):
    ...
    for attr in vars(args):
        ...
        # Deduplicate and validate the extant files
        if attr == 'extant' and val:
            dic = {}

            # Collect all files, deduplicate by path
            all_files = set()  # Use a set to store unique file paths
            rest = []

            for elem in val:
                key, *v = elem.split('=', 1)
                if v:
                    if v[0] not in all_files:  # Deduplicate here
                        dic[key] = v[0]
                        all_files.add(v[0])
                else:
                    rest.append(key)

            # Assign unique keys to remaining files
            key = 'A'
            for elem in rest:
                while key in dic:  # Avoid existing keys
                    key = chr(ord(key) + 1)
                if elem not in all_files:  # Deduplicate here as well
                    dic[key] = elem
                    all_files.add(elem)
            ...
            for key in dic:
                if not os.path.isfile(dic[key]):
                    err = f'** ERROR: file "{dic[key]}" does not exist **'
                    good = False
            ...
            val = ' '.join(val)  # Join formatted strings
            reported['extant'] = val
            setattr(args, attr, dic)  # Save parsed dictionary to args
        ...
        if attr == 'cost':
            ...
            for key in ext:
                ...
                comp[key] = []
                runs[key] = []
                mcols[key] = len(csv.DictReader(
                    open(ext[key], 'r')).fieldnames) - 1
            ...
            for elem in val:
                ...
                if v:
                    ...
                    if key:
                        ...
                        if start:
                            # filter ranges which start beyond the end
                            if start <= mcols[key]:
...
parser.add_argument(
    '-r', '--scores', metavar='scores.csv',
    help='direct scores for most parsimonious solutions(s) to given (csv) file')
...
parser.add_argument(
    '-b', '--solns', metavar='solns.csv',
    help='direct parsimonies (in terms of transitions) to a give (csv) file')
...
for node in tree.traverse('preorder'):
    ...
    for i in range(m):
        ...
        if node.is_root():
            ...
            state[node.name].append(a[i][numpy.argmin(list(
                score[node.name][i][j] +
                cost[i][(a[i][j], state[node.up.name][i])]
                for j in range(k[i])))])
...
# dump scores at the end
if args.scores:
    scores_file = open(args.scores, 'w')

    scores = list(min(score[tree.get_tree_root().name][i][j]
                      for j in range(k[i]))
                  for i in range(m))

    print('Character', *char, sep=',', file=scores_file)
    print('Score', *scores, sep=',', file=scores_file)
...
if args.nexus:
    ...
    # all names, with NHX annotations
    ts = tree.write(format=8, features=['label'])
...
def traceback(i, node, q):
    ...
    # recursive case: deal with internal node
    mini = min(score[node.name][i][j] + cost[i][(a[i][j], a[i][q])]
               for j in range(k[i]))
    for j in range(k[i]):
...
for i in range(m):
    ...
    if b > 1:
        print('  ', char[i], ': ', b, ' possibilit', 'ies' if b >
              1 else 'y', '.. ', sep='', end='', flush=True)
    ...
    ns = len(solns)
    print('Sp.\\Soln.', *range(1, ns+1), sep=',', file=solnsfile)
    ...
    for i in range(m):
        for si in range(nsolns[char[i]]):
            for a, b in sorted(unit[i][si]):
                ...
                c = len(unit[i][si][(a, b)])
                bs = '; '.join('{} -> {}'.format(x, y)
                               for x, y in unit[i][si][(a, b)])
                print(char[i], si+1, t, c, bs, sep=',', file=unit_h)

    if args.solns:

        solns_h = open(args.solns, 'w')
        print('Char,Soln,Size,Transitions', file=solns_h)
        ...
        for i in range(m):
            for si in range(nsolns[char[i]]):
                ...
                ts = []
                for a, b in u:
                    ts += ['{}:{}->{}'.format(y, a, b) for x, y in u[(a, b)]]

                s = len(ts)
                ts = '{' + '; '.join(ts) + '}'
                print(char[i], si+1, s, ts, sep=',', file=solns_h)

#
# from unit transitions occurrences (unit), compute pairwise
# transition co-occurences and output to (filename)


def pairwise_to_file(filename, m, char, nsolns, unit, branches=False):

    # collapse unit transition occurrences into single "consensus map"
    c_map = {}
    for i in range(m):
        ...
        for si in range(nsolns[char[i]]):
            ...
            for a, b in unit[i][si]:
                c[(a, b)] += unit[i][si][(a, b)]

        # store as multiset for (effectively) weighted Jaccard index
        c_map[i] = {}
        for a, b in c:
            c_map[i][(a, b)] = Multiset(c[(a, b)])

    # dump pairwise transition co-occurrence to file
    pairs_h = open(filename, 'w')
    header = ['Char_1', 'Char_2', 'Transition_1', 'Transition_2']
    header += ['Count', 'Correlation', 'Jaccard']
    header += ['C_Map_1 & C_Map_2', 'C_Map_1', 'C_Map_2', 'C_Map_1 | C_Map_2']
    ...
    for i in range(m):
        ...
        for j in range(i):
            ...
            for ai, bi in sorted(cmi):
                A = cmi[(ai, bi)]

                for aj, bj in sorted(cmj):
                    B = cmj[(aj, bj)]

                    inter = A & B

                    # note only the non-zero correlations
                    if not inter:
                        continue

                    union = A | B
                    # correlation is weighted Jaccard index
                    correlation = len(inter) / float(len(union))
                    jaccard = len(set(inter)) / float(len(set(union)))

                    ti = '{} -> {}'.format(ai, bi)
                    tj = '{} -> {}'.format(aj, bj)

                    aib = '; '.join('{} -> {}'.format(x, y) for x, y in inter)
                    a = '; '.join('{} -> {}'.format(x, y) for x, y in A)
                    b = '; '.join('{} -> {}'.format(x, y) for x, y in B)
                    aub = '; '.join('{} -> {}'.format(x, y) for x, y in union)

                    coll = char[i], char[j], ti, tj, len(
                        inter), correlation, jaccard
                    if branches:
                        coll += aib, a, b, aub
...
if args.pairwise:
    ...
    pairwise_to_file(args.pairwise, m, char, nsolns, unit, True)
