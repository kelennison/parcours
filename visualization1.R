...
pairwise_df <- pairwise_df %>%
  mutate(C_Map_1 = str_extract(C_Map_1, "(?<=->).*"),  # Extract everything after '->'
...
# Debugging: Print out the modified C_Map_1 and C_Map_2
print("Modified C_Map_1:")
print(unique(pairwise_df$C_Map_1))

print("Modified C_Map_2:")
print(unique(pairwise_df$C_Map_2))
...
tree_plot <- tree_plot + 
  geom_nodelab(aes(label = pairwise_df$Char_1_annotated[match(label, pairwise_df$C_Map_1)]), 
               vjust = -3, hjust = 1.5, color = "blue")  # Directly use pairwise_df without filtering
...
tree_plot <- tree_plot + 
  geom_nodelab(aes(label = pairwise_df$Char_2_annotated[match(label, pairwise_df$C_Map_2)]), 
               vjust = -1.5, hjust = 1.3, color = "red")  # Directly use pairwise_df without filtering
...
tree_plot <- tree_plot + 
  geom_tiplab(aes(label = pairwise_df$Char_1[match(label, pairwise_df$C_Map_1)]), 
              vjust = -3, hjust = 2, color = "blue")  # Adjust vjust and hjust for better alignment1
