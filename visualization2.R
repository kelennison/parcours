...
pairwise_df <- pairwise_df %>%
  mutate(
    C_Map_1 = str_extract_all(C_Map_1, "(?<=->)[\\w]+"),
...
for (i in seq_len(nrow(pairwise_df))) {
  ...
  tree_plot_new <- tree_plot_new +
    geom_nodelab(
      aes(label = ifelse(label %in% names(char1_counts),
        paste(char1_labels, "(", char1_counts[as.character(label)], "x)", sep = ""), ""
      )),
      vjust = -3.5, hjust = 1.2, color = "red"
    ) +
    geom_nodelab(
      aes(label = ifelse(label %in% names(char2_counts),
  ...
  if (nrow(current_node_coords) == 1 && nrow(parent_node_coords) == 1) {
    ...
    if (num_annotations > 1) {
      for (i in 0:(num_annotations - 1)) { # Adjust to n-1 lines
        ...
        tree_plot <- tree_plot +
          annotate("segment",
            x = x_start, xend = x_end, y = y_line, yend = y_line,
...
for (j in seq_len(nrow(annotation_counts))) {
  ...
  if (annotation_counts$source[j] == "Char_1") {
    y_base <- tree_data_new %>%
      filter(label == node_n) %>%
      pull(y) - 0.1 # Starting y position for Char_1
    tree_plot_new <- draw_parallel_lines(tree_plot_new,
      node_n = node_n,
